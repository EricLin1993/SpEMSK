...
% run(fullfile(packageDir, 'mirt-main', 'setup.m')); 
...
%% Use synthetic dynmic MSK image to generate multi-circle motion data in kSpace
...
img_Non_Reordering = nufft_Open_lep(kSpace_CC, traj_Mo, baseResolution, fovx_mm, 'down_sampling_factor', 1,'subsets',Subsets_SE);  
img_Non_Reordering = img_Non_Reordering/max(abs(img_Non_Reordering(:)));
clear kSpace_CC
...
for PC =1:4
    ...
    % img_SpE_Reordering = tf_nufft_radial_recon_lep(kSpace_CC, traj_SEOrd, baseResolution, fovx_mm, 'down_sampling_factor', 1,'subsets',Subsets_SE);% This is not open-sourced, and use nufft_Open instead. 
    img_SpE_Reordering = nufft_Open_lep(kSpace_CC, traj_SEOrd, baseResolution, fovx_mm, 'down_sampling_factor', 1,'subsets',Subsets_SE); %, ,'dcf_method','pipe', 'dcf_iters', 30
    clear kSpace_CC
    SpE_Rec_PC{PC} = img_SpE_Reordering/max(abs(img_SpE_Reordering(:)));
